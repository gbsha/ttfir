...
async def test_minus_function(dut):
    ...
    output_expected = [0] * (N_TAPS + 2 + 1) + [-(x * 2 * 31)//32 for x in range(-15, 16)]
    dut._log.info("start")
    clock = Clock(dut.clk, 10, units="us")
    cocotb.start_soon(clock.start())
    dut._log.info("reset")
...
async def test_maximum_value(dut):
    ...
    output_expected = [0] * (N_TAPS + 2 + 1) + [i * 31 * 31 // 32 for i in range(1, N_TAPS + 1)]
    dut._log.info("start")
    clock = Clock(dut.clk, 10, units="us")
    cocotb.start_soon(clock.start())
    dut._log.info("reset")
...
async def test_minimum_value(dut):
    ...
    output_expected = [0] * (N_TAPS + 2 + 1) + [-32 * 31 * i // 32  for i in range(1, N_TAPS + 1)]
    dut._log.info("start")
    clock = Clock(dut.clk, 10, units="us")
    cocotb.start_soon(clock.start())
    dut._log.info("reset")
